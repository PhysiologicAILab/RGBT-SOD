...
class SSLNet(nn.Module):
    ...
    def forward(self, rgb, ti):
        ...
        F5 = self.channel5(A5_t,A5) 
        F4 = self.channel4(A4_t,A4) 
        F3 = self.channel3(A3_t,A3) 
        F2 = self.channel2(A2_t,A2)
        F1 = self.channel1(A1_t,A1)
        ...
        if self.training:
            ...
            out_a5_t = torch.nn.functional.interpolate(self.convt_g(A5_t), rgb.size()[2:], mode="bilinear")

            return out,out_a5,out_a5_t,A5,A5_t
